...
//			}
	...
	public String setStatus(String id) {
		...
		String status = "";
		if(Global.ENABLE.equals(category.getInMenu())){
			category.setInMenu(Global.DISABLE);
			status = "禁用成功";
		} else {
	...
	public String selectService(String string) {
		...
		return catDao.selectService(string);
	}
	
	public Page<Category> findPage(Page<Category> page, Category category) {
		category.setPage(page);
		page.setList(catDao.findListForPage(category));
		return page;
	}
	
	public List<Category> findListForPage(Category category){
		return catDao.findListForPage(category);
	}
	...
	public void sort(String sourceId, String destinationId) {
		Category categorySource = dao.get(sourceId);
		Category categoryDestination = dao.get(destinationId);
		int temp = categorySource.getSort();
		categorySource.setSort(categoryDestination.getSort());
		categoryDestination.setSort(temp);
		dao.update(categoryDestination);
		dao.update(categorySource);
	}
    
	@Transactional(readOnly=false)
	public void deleteByLogic(Category category) {
		dao.delete(category);
