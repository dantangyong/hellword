...
//	public List<Category> findByUserId(Long userId);
	public void setShowOrHide(String s);

	public String selectService(String aid);
	public List<Category> findListForPage(Category entity);
    
	@Select("select name from mht_portal.cms_category where del_flag = '0' and name = #{name} ")
	public Category getByName(String name);
	
	//前台
	List<Category> selectAll();

	List<Category> selectChannel(String name);

	List<Category> selectAllChannel();

	List<Category> selectHotChannel(int index);
}
