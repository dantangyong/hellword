...
public class CmsUtils {
    ...
    private static TagManageService tagManageService = SpringContextHolder.getBean(TagManageService.class);

	private static final String CMS_CACHE = "cmsCache";
	private static final String TAGMANAGE_CACHE = "tagManageCache";
    ...
    /**
	 * @Title:  getTagManage
	 * @Description: 获得网站tagManage信息:logo,页脚
	 * @version 2017-11-13
	 * @author dty
	 */
    public static TagManage getTagManage( ){
    	TagManage tagManage = (TagManage) CacheUtils.get(TAGMANAGE_CACHE, "tm");
		if (tagManage == null){
			TagManage tm = new TagManage();
			tm.setLogoAble("1");
			List<TagManage> tagManages = tagManageService.findList(tm);
			if(tagManages.size()>0){
				tagManage = tagManages.get(0);
			} else{
				tagManage = new TagManage();
			}
			CacheUtils.put(TAGMANAGE_CACHE, "tagManage", tagManage);
		}
    	return tagManage;
    }
}