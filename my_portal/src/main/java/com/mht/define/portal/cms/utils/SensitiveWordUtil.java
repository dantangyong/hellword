...
public class SensitiveWordUtil {
     ...
     */
    public static synchronized void init(Set<SensitiveWord> sensitiveWordSet) {
     ...
     * @param sensitiveWordSet 敏感词库
     */
    private static void initSensitiveWordMap(Set<SensitiveWord> sensitiveWordSet) {
        //初始化敏感词容器，减少扩容操作
        ...
        while (iterator.hasNext()) {
            ...
            for (int i = 0; i < s.getSensitiveName().length(); i++) {
                //转换成char型
                char keyChar = s.getSensitiveName().charAt(i);
                //库中获取关键字
                ...
                if (i == s.getSensitiveName().length() - 1) {
                    //最后一个
                    nowMap.put("isEnd", "1");
                }
            }
