...
<script type="text/javascript">
	function refresh() {//刷新
		window.location = "${ctx}/portal/auth/user";
	}
</script>
</head>
<body class="gray-bg">
			...
			callback : {
				onClick : function(event, treeId, treeNode) {
					...
					if (treeNode.obj == "user") {
						getOfficeList(id,treeNode.name);
					} else {
// 						$('#officeContent').attr(
// 								"src",
...
// 										+ id + "&office.name=" + treeNode.name);
		...
		function ajaxDataFilter(treeId, parentNode, responseData) {
			if (responseData) {
				if (initValue == 1) {
					initValue = 2;
					getOfficeList(responseData[0].id,responseData[0].name);
// 					initMsg(responseData[0].id, responseData[0].name);
		...
		function initMsg(id, name) {
			$('#officeContent').attr(
					"src",
					"${ctx}/portal/auth/userList?id="+ id + "&name=" + name);
		}
		...
		function getOfficeList(id,name){
	        $.ajax({
	        type:"POST",
	        url:"${ctx}/portal/auth/userList",
	        data: {
	            "id":id,
	            "name":name
	        },
